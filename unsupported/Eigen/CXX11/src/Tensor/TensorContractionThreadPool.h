...
namespace Eigen {
/*模板函数的声明,Indices
是一个类型参数，用于指定张量收缩操作中的索引序列。在张量收缩中，每个索引对应两个张量中的相同维度，这些维度将进行求和。Indices
指定了这些索引的顺序。 LeftArgType 是一个类型参数，用于指定张量收缩操作中的左侧张量类型。RightArgType
是一个类型参数，用于指定张量收缩操作中的右侧张量类型。 OutputKernelType 是一个类型参数，用于指定输出结果的类型。*/
template <typename Indices, typename LeftArgType, typename RightArgType, typename OutputKernelType>
/*结构体模板的声明,两个模板参数。前一个是类型参数，用于指定要执行的张量收缩操作的模板类型。ThreadPoolDevice用于指定计算设备的类型。指定了使用线程池来执行计算操作。
summary：这个结构体模板的作用是评估张量收缩操作，进行计算并生成输出张量。它包含一些成员函数，如
Eval()，用于执行张量收缩计算，以及一些成员变量，如 m_op 和 m_thread_pool，
...
struct TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType, OutputKernelType>,
                       ThreadPoolDevice> :
    /*公共继承语句，它继承了 TensorContractionEvaluatorBase 类的模板实例化版本作为其基类。类型参数是
    TensorEvaluator，它表示使用线程池设备来评估张量收缩操作。 这个语句的作用是将 TensorEvaluator
    类模板的实例化版本作为基类，以便使用基类中的某些成员函数和变量，从而简化代码实现。*/
  ...
  Scalar 是一个定义在 TensorContractionOp
  模板类中的类型别名，它表示张量的标量类型。这个类型别名的作用是方便地获取张量的标量类型，并简化代码实现。
  ...
  // static_cast<int>(Layout) == static_cast<int>(ColMajor) 判断了 Layout 是否等于 ColMajor，如果满足条件则
  // EvalLeftArgType 的类型为 LeftArgType，否则为 RightArgType。 std::conditional_t
  // 的作用是在编译期间选择不同的类型，这在模板编程中非常有用。使用 typedef 关键字将 std::conditional_t 返回的类型定义为
  // EvalLeftArgType 的别名，方便后续代码的使用。
  ...
  static constexpr int LDims =
      // 通过 TensorEvaluator<EvalRightArgType, Device>::Dimensions 获取到的一个数组类型，然后使用 internal::array_size
      // 获取到该数组的大小（即元素个数），最终将其作为一个编译期常量的整数类型变量的值。
  ...
  // 定义三个类型别名，LhsScalar 是 EvalLeftArgType::Scalar 的去除 const 限定符后的类型，RhsScalar 是
  // EvalRightArgType::Scalar 的去除 const 限定符后的类型。
  ...
  // internal::gebp_traits<LhsScalar, RhsScalar> 是一个模板元函数，用于根据左右两个张量的标量类型推导出对应的 GEBP
  // 特征类型。
  ...
  // 模板类 TensorEvaluator 的构造函数，接受两个参数。表示张量表达式的类型 XprType，表示计算设备的类型
  // Device，并将其传递给 Base 类的构造函数。 Base 则是它的基类，首先将 op 和 device 分别传递给 Base
  // 类的构造函数，以完成对基类的初始化。
  ...
  // 作用是计算张量表达式的乘积，并将结果存储到 buffer 中。在具体实现中，它调用了 evalProductImpl 函数，传递了
  // NoCallback 类型的对象作为回调函数。
  ...
  void evalProductImpl(Scalar* buffer, DoneCallback done) const {
    ...
    如果选择异步模式，函数会在堆上分配一个 `Context` 对象，并在所有任务完成后调用提供的回调函数，然后从堆上删除
    `Context` 对象。 `EvalParallelContext` 和 `EvalShardedByInnerDimContext`
    拥有执行张量收缩所需的所有状态和临时缓冲区。它们负责在收缩完成后清理这些缓冲区。*/
    ...
    首先，代码计算了一组粗略的参数，包括行/列分片策略。由于此时线程数未知，因此使用了一个默认值
    2，主要是为了判断是否应该使用并行化。 其中，行/列分片策略会在后续的计算中再次进行调整和优化。*/
    // 根据张量表达式的维度大小以及默认的线程数 2 来判断是否应该采用列分片策略。
    // 具体来说，shardByCol
    // 函数会根据维度大小和线程数来计算一个分片阈值，如果分片阈值小于维度大小，则采用列分片策略；否则采用行分片策略。
    ...
    // across k dimension.
    /*作用是计算最优线程数。这里使用 bk 而不是 k 来计算最优线程数，因为我们希望计算的是可并行计算的数量，而在 k
    维度上的计算是不可并行的。 计算最优线程数的方法是根据当前系统的 CPU
    核心数量、内存带宽、缓存大小等一系列硬件参数，以及之前计算得到的内核块大小和其他参数，来估计最优线程数。
    ...
    */
    ...
    // 调用 TensorCostModel<ThreadPoolDevice>::numThreads
    // 函数来计算最优线程数，该函数考虑了硬件参数、内核块大小、分片策略等因素。
    int num_threads =
        TensorCostModel<ThreadPoolDevice>::numThreads(static_cast<double>(n) * m, cost, this->m_device.numThreads());
    // 用 numThreadsInnerDim
    // 函数来计算在内部维度上的最优线程数。这个函数根据内核块大小、内存带宽、缓存大小等因素来计算最优线程数，并返回一个整数值。
    ...
    if (shardByInnerDim(m, n, k, num_threads, num_threads_by_k)) {
      ...
      if (IsEvalInSyncMode) {
        ...
        // runAsync 方法会在一个新的线程中异步执行计算过程，以避免阻塞主线程。在异步执行完成后，会调用 done
        // 回调函数来通知计算结果。
        auto* ctx =
    ...
    处理一些特殊情况，例如当张量乘积中的一个维度大小为 1 时，或者最优线程数为 1
    时，直接采用顺序计算的方式来完成计算，而不是采用并行计算。
    这种特殊处理可以提高计算的效率和正确性，避免不必要的并行计算开销。*/
    // 检查张量乘积的第二个维度大小 n 是否为 1，如果是，则直接将最优线程数设置为
    // 1，因为在这种情况下，并行计算没有任何优势。
    ...
    // consecutive kernels).
    // 允许更好地使用缓存。通过在几个连续的内核中重复使用相同的打包右手边（packed
    // rhs）数据，可以将数据保留在缓存中，减少昂贵的内存访问的需求。
    ...
    */
    ...
    // 如果 sharding_dim_tasks 大于等于 oversharding_factor 乘以 num_worker_threads，则 parallelize_by_sharding_dim_only
    // 的值为 true，表示仅在分片维度上进行并行化； 否则其值为 false，表示需要在其他维度上进行并行化。
    ...
    /*这段代码的作用是根据任务数量和可用的线程数来决定是否同时对 lhs 和 rhs 进行打包，并行化打包和内核执行。或者先对 lhs
    进行打包，等 lhs 打包完成后再进行 rhs 的打包，并行化内核执行。 其中，如果按行分片，则 lhs 和 rhs
    分别表示原始矩阵的行和列，如果按列分片，则 lhs 和 rhs 分别表示原始矩阵的列和行。如果按列分片，则需要先对 lhs
    进行打包，然后再对 rhs 进行打包。如果按行分片，则需要先对 rhs 进行打包，然后再对 lhs 进行打包。
    ...
    if (IsEvalInSyncMode) {
      // 如果处于同步模式，则使用 SyncEvalParallelContext 来进行计算。代码使用 TENSOR_CONTRACTION_DISPATCH
      // 宏来分派计算任务。
#define CONTEXT_ARGS                                                                                          \
...
#undef CONTEXT_ARGS
  ...
  // 定义了一个名为 NoCallback 的结构体，用于表示一个空的回调函数。这个结构体没有任何成员变量，只定义了一个 operator()
  // 函数。 当这个回调函数被调用时，它会抛出一个异常，因为空回调函数不应该被调用。
  ...
  /*解释了上下文对象 Context 的作用。Context
  对象是用来协调同步/异步并行收缩运算的。当以异步模式执行时，它拥有可能被块打包和核心任务访问的所有共享状态。 Context
  对象管理并行计算过程中的所有状态和资源，包括块状矩阵的分配和释放、线程池的管理、任务调度和同步等待等。
  在同步模式下，Context 对象的作用相对较小，因为所有计算都是在调用上下文对象的 operator()
  函数时立即执行的，不需要管理任何状态或资源。 但在异步模式下，Context
  对象的作用就非常重要了，因为它需要管理并发执行的多个任务，保证它们能够正确地访问和修改共享状态，以及避免出现竞态条件和死锁等并发问题。*/
  ...
  class EvalParallelContext {
   public:
    ...
    typedef internal::TensorContractionInputMapper<RhsScalar, Index, internal::Rhs, RightEvaluator, right_nocontract_t,
        ...
        RhsMapper;
    // OutputMapper
    // 表示输出矩阵的映射器类型。这个映射器用于将计算结果存储到输出矩阵中。它的模板参数包括矩阵元素类型、索引类型、矩阵存储顺序等。
    typedef internal::blas_data_mapper<Scalar, Index, ColMajor> OutputMapper;
    // TensorContractionKernel
    // 表示张量收缩运算的核心计算单元类型。这个计算单元封装了一系列计算操作，包括块状子矩阵的打包、内积计算、累加等。
    ...
    EvalParallelContext(const Self* self, int num_threads, Scalar* buffer, Index tm, Index tn, Index tk, Index bm,
                        ...
                        bool shard_by_col, bool parallel_pack, bool parallelize_by_sharding_dim_only,
                        DoneCallback done)
        : created_by_thread_id_(std::this_thread::get_id()),  // 表示创建该对象的线程 ID。
          ...
          rhs_(self->m_rightImpl, self->m_right_nocontract_strides, self->m_j_strides,
               ...
               self->m_k_strides),  // 表示左矩阵和右矩阵的映射器对象。它们用于将矩阵分块、打包成连续的内存块，并在计算时调用
                                    // BLAS 库进行内积计算等。
          ...
          tensor_contraction_params_(
              self->m_tensor_contraction_params),  // 张量收缩的参数对象，包括矩阵形状、收缩维度等信息。
          ...
          // 根据 shard_by_col 的值仅使用左矩阵或右矩阵的本地存储。shard_by_col 为
          // true，则使用右矩阵的本地存储，否则使用左矩阵的本地存储。
      ...
      // 如果启用了 parallel_pack，则表示在打包操作中使用并行化，而如果启用了
      // parallelize_by_sharding_dim_only，则表示只在划分维度上并行化计算。
      ...
      for (Index x = 0; x < P; x++) {
        ...
        st P - 1 slices will receive only
                   ...
                   // 每个线程需要发送的通知数量。
                   state_switch_[x] =
            x == 0 ? 1  // 第一个 k slice 只需要发送 nm_ + nn_ 条通知
   ...
   private:
    ...
    // packing tasks.
    /*同一 k 块相关的块可以并行运行，因为它们写入不同的输出块。因此，我们在每个 k 切片内并行化，这为我们提供了 m x n
    的并行级别。 在我们开始执行与第 k 切片相关的任何内核之前，我们需要发出 m 个 LHS 打包任务和 n 个 RHS 打包任务。*/
    //
    ...
    /*然而，当我们完成第 k 切片的内核时，存在瓶颈（最后只有一个可运行的内核）。为了缓解这个瓶颈，我们允许第 k 切片和第
    k+1 切片的内核并行运行。 请注意，(m, n, k) 和 (m, n, k+1) 内核写入相同的输出块，因此它们不能并行运行。*/
    //
    ...
    用于 packed_lhs 或 packed_rhs（当 shard_by_col = false 或 true
    时）。这些内存不能传递给可能在不同线程上执行的内核。*/
    //
    ...
    /*实际上，当我们准备为第 K 切片的分片维度（如果 shard_by_col==true，则为 rhs）打包内存时，所有第 K-1
    切片的内核已经计算完成（99% 的情况下），
    我们可以将数据打包到线程本地存储中，并保证所有内核将在同一线程中立即执行。这显著提高了 L1
    缓存命中率，并减少了内存总线的压力。*/
    //
    ...
    /*仍然可能出现第 K 切片的内核在第 K-1 内核完成之前就已准备好的情况，因此我们必须分配“全局”packed_lhs_ 和
    packed_rhs_， 以允许内核在稍后在不同于用于打包的线程上执行。*/
    ...
    std::atomic<int> num_thread_local_allocations_;  // 跟踪已经分配的线程本地存储的数量，std::atomic
                                                     // 类型可以确保并发线程安全地访问该变量，避免出现数据竞争。
    const int
        thread_local_capacity;  // 表示线程本地存储的容量。是每个线程独有的存储空间，可以用于存储线程私有的数据，避免线程间的数据竞争。
    ...
    /*如果系统中的唯一线程数小于或等于线程池中的线程数，则我们将使用上面定义的预分配的 Lhs/Rhs
     * 块。在此之后，我们将回退到动态内存分配。*/
    ...
    /*ThreadLocalBlocksInitialize
     * 可调用对象进行自定义线程本地块的初始化，如果可能会重用预分配的缓冲区，否则将动态分配新的内存。*/
    //
    ...
    */
    ...
    如果m == 0（n ==
    0）的内核已准备好运行，则保证具有更大m（n）值的所有内核也已准备好，因为我们对所有K个切片按相同顺序执行它们。*/
    ...
    void pack_rhs(Index n, Index k) {
      ...
      for (Index n1 = n * gn_; n1 < nend; n1++) {
        if (!TensorContractionKernel::HasBeta && k == 0) {
          ...
          // 只有在缩并核不支持 `beta` 时，才需要进行清零操作。否则，我们将在第一次调用
          // `TensorContractionKernel::invoke()` 时传递 beta = 0.0。 在 10000x2x10000
          // 的情况下，清零操作很容易占用一半的时间。
      ...
      if (shard_by_col_) {
        for (Index n1 = n * gn_; n1 < nend; n1++) {
          for (Index m1 = m * gm_; m1 < mend; m1++) {
            ...
            /*调用了张量乘法内核的 invoke 函数，用于执行 LHS 与 RHS 的乘法操作，并将结果写入到 output_mapper
             * 指定的输出内存区域中。*/
  ...
  struct EvalShardedByInnerDimContext {
    EvalShardedByInnerDimContext(const Self* self, int num_threads, Scalar* result_buffer, Index m_size, Index n_size,
      ...
      for (Index block_idx = 0; block_idx < num_blocks; ++block_idx) {
        ...
        // 对于其他块，即操作数矩阵的小块，缓冲区通过 evaluator->m_device.allocate(buffer_size_bytes) 分配在 GPU
        // 设备上。
   ...
   private:
    ...
    void evalAsync(Index start_block_idx, Index end_block_idx) {
      ...
      if (v == 1) {
        ...
        // 注意：如果在删除当前上下文之前调用 `done` 回调，会尝试释放上下文中捕获的 Self*
        // 指针，导致在析构函数中失败尝试释放临时缓冲区 将 `done` 回调从上下文中移动出来，以避免在删除上下文之前调用它
  ...
  static bool shardByInnerDim(Index m, Index n, Index k, int num_threads, int num_threads_by_k) {
    ...
    if (n == 1 ||                          // If mat*vec or...是mat*vec
        ...
        num_threads_by_k < num_threads ||  // sharding by k gives less parallelism or... 按k分片会导致并行度降低
